...
class st_gcn_tony(BaseModel):
    def __init__(self, config: DictConfig):
        ...
        A = np.load("adj_matrix_cnslab_fmri.npy")
        Dl = np.sum(A, 0)
        num_node = A.shape[0]
        ...
        if config.model.edge_importance_weighting:
        ...
        self.fcn = nn.Conv2d(64, output_dim, kernel_size=1)
        # self.sig = nn.Sigmoid()
