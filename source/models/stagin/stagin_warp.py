from .stagin import ModelSTAGIN
...
class staginWarp(BaseModel):
    def __init__(self, config: DictConfig):
        super().__init__()
        self.config = config
        self.model = ModelSTAGIN(config)
        self.device = torch.device("cuda")
        
    def forward(self,
                a, v, sp, t):
        out, _, _, _ = self.model(v.to(self.device), a.to(self.device), t.to(self.device), sp)
        return out
