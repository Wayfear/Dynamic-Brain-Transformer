...
class ModelSTAGIN(nn.Module):
    def __init__(self, config, input_dim=360, hidden_dim=128, num_heads=1, num_layers=4, sparsity=30, dropout=0.5, cls_token='sum', readout='sero', garo_upscale=1.0):
        super().__init__()
        self.config = config
        ...
        num_classes = output_dim = 1 if config.dataset.regression else config.dataset.num_classes
